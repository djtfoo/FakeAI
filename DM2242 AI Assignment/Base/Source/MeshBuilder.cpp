...
*/
/******************************************************************************/
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, Color color, float length, float texTile)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	v.color = color;
	v.normal.Set(0, 0, 1);
    v.texCoord.Set(1.0f * texTile, 0);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f * length, 0.5f * length,0);
	v.color = color;
	v.normal.Set(0, 0, 1);
    v.texCoord.Set(1.0f * texTile, 1.0f * texTile);
	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f * length, 0.5f * length,0);
	v.color = color;
	v.normal.Set(0, 0, 1);
    v.texCoord.Set(0, 1.0f * texTile);
	vertex_buffer_data.push_back(v);
